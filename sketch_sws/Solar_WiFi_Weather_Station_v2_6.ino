...
// PubSubClient (MQTT) wurde entfernt ‚?" Station arbeitet ausschlieﬂlich mit REST-API
...
StationConfig cfg;  // globale Instanz ‚?" √ºberall im Sketch verwendet
...
// EEPROM-Magic-Bytes: zeigen an, dass g√ºltige Daten gespeichert sind
...
void loadConfig() {
  // Erst Defaults setzen, dann ggf. mit EEPROM-Werten √ºberschreiben
  applyDefaults();
  ...
  if (!valid) {
    Serial.println("EEPROM leer/ung√ºltig ‚?" verwende Compile-Zeit-Defaults.");
    return;
  }
  ...
  if (deserializeJson(doc, buf) != DeserializationError::Ok) {
    Serial.println("EEPROM-JSON ung√ºltig ‚?" verwende Compile-Zeit-Defaults.");
    return;
  }

  // Nur vorhandene EEPROM-Felder √ºberschreiben; fehlende behalten den Default
...
// Der ESP √∂ffnet einen Access Point (SSID: SWS-Config) und stellt
...
<h2>WLAN</h2><table>)";
    ...
    html += fieldInt("H√∂he √º. NN (m)", "elevation", cfg.elevation);
...
void setup() {
  ...
  // Konfigurations-Portal pr√ºfen: Button (CONFIG_BUTTON_PIN) beim Start LOW?
  pinMode(CONFIG_BUTTON_PIN, INPUT_PULLUP);
  delay(10);
  if (digitalRead(CONFIG_BUTTON_PIN) == LOW) {
    startConfigPortal();   // kehrt nicht zur√ºck (Neustart am Ende)
  }
...
float getTemperature() {
  ...
  for (int attempt = 1; attempt <= 3; attempt++) {
    ...
    if (attempt < 3) delay(200);  // kurz warten vor n√§chstem Versuch
  }
  Serial.println("DS18B20 Fehler: alle 3 Versuche fehlgeschlagen! Pr√ºfe Verkabelung und 4.7kŒ© Pull-up auf D7.");
  return -88;
}
#endif
